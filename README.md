...
- remove images having CP < 0.02 ; very small CP values can give unreliable estimates. This is because PAI values greatly with small CP, and small CP values can be due to DCP image, EzPAI limitations. Otherwise PAI change between hourly obs can be unrealiable, jump. CP < 0.015 should also be ok; we tested it over MA stations and results were nearly identical. But CP < 0.015 may be better for when EzPAI results are ? 5.0 (most of our sites did not reach that). 
...
LICOR measurment over 200 m x 200 m region near station on 7/19/2022 gave PAI = 4.16. Time series for 2022 from EzPAI default settings + "Suggested postprocessing" gives 4.09 for the average (see image below). Please note that the image also gives the average of daily standard deviations rather than the standard deviation of the time series, thus there is no result on days with only one observation, and often the metric may be based only on few imaged. Nonetheless, this value is usually also similar to the std of the daily averages or all valid hourly PAI during Jun 1 to Sep 30, here 0.24 and 0.33, respectively.
