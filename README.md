...
Naming convention: the .csv and hist*.jpg output results are named using the input folder name. There was a different file organization between SMAPVEX19-22 MA and MB sites. For MA, stations are named 420 etc, and all the input data are in the same folders. For MB, folders are organized by when a field person manually downloaded all the data to a memory card. For example one of the example given, the field person did a download on 2020-6-29 of data going back to the beginning of the experiment (~May 2019). For MB this was helpful, given that many timestamps were reset in 2022, but the field person collection date information facilitated assigning correct dates. The different file organization stem from different persons managing the collections.
